...
# ══════════════════════════════════════════════════════════════════════════════
# 8. SUBSCRIPTION MODEL  (Owner Pricing Plans)
# ══════════════════════════════════════════════════════════════════════════════
class Subscription(models.Model):
    """
    Tracks a station owner's subscription plan.

    Plans:
      - starter : 500 bookings/month  for ₹1,000
      - pro     : 1,200 bookings/month for ₹2,000

    Revenue model:
      - No commission charged within the plan's booking limit.
      - After exceeding the limit (e.g. the 501st booking on Starter),
        a 5% commission is deducted from each subsequent booking's amount.
      - 'bookings_remaining' is decremented each time a booking is completed
        (status changed to 'completed' by the owner).

    When bookings_remaining reaches 0, subsequent completed bookings
    generate a commission entry tracked by 'commission_earned' (platform side)
    and 'commission_owed' (what the owner owes).
    """

    PLAN_CHOICES = [
        ('starter', 'Starter — 500 Bookings / ₹1,000'),
        ('pro',     'Pro — 1,200 Bookings / ₹2,000'),
    ]
    ...
    PLAN_CONFIG = {
        'starter': {'bookings': 500,  'price': 1000},
        'pro':     {'bookings': 1200, 'price': 2000},
    }

    COMMISSION_RATE = 5  # 5% commission after plan limit
    ...
    # Total bookings included in the plan
    bookings_total = models.PositiveIntegerField()

    # Remaining bookings (decremented on each completed booking)
    bookings_remaining = models.PositiveIntegerField()

    # How many bookings have been used from this plan
    bookings_used = models.PositiveIntegerField(default=0)

    # Commission tracking (accumulated when bookings_remaining == 0)
    commission_owed = models.DecimalField(
        max_digits=12, decimal_places=2, default=0,
        help_text='Total commission the owner owes to the platform (5% of over-limit bookings)'
    )
    commission_bookings = models.PositiveIntegerField(
        default=0,
        help_text='Number of bookings that exceeded the plan limit'
    )
    ...
    def __str__(self):
        return f"{self.owner.username} — {self.get_plan_display()} — {self.status}"
    ...
    def is_over_limit(self):
        """Returns True if all included bookings have been used up"""
        return self.bookings_remaining <= 0

    def deduct_booking(self, booking_amount=0):
        """
        Called when a booking is marked 'completed'.
        - If bookings remain, decrement the counter.
        - If over limit, calculate and add 5% commission.

        Returns:
            dict with 'commission' (float) and 'was_over_limit' (bool)
        """
        result = {'commission': 0.0, 'was_over_limit': False}

        if self.bookings_remaining > 0:
            self.bookings_remaining -= 1
            self.bookings_used += 1
        else:
            # Over limit — charge 5% commission
            commission = float(booking_amount) * self.COMMISSION_RATE / 100
            self.commission_owed += commission
            self.commission_bookings += 1
            self.bookings_used += 1
            result['commission'] = round(commission, 2)
            result['was_over_limit'] = True
