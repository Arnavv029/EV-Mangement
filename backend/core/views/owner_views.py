...
from core.models import Station, TimeSlot, Booking, Review, Subscription
...
def owner_update_booking_status_view(request, pk):
    ...
    if serializer.is_valid():
        ...
        # ── SUBSCRIPTION: Deduct booking when owner confirms the booking ──
        ...
        if new_status == 'confirmed':
            ...
            if active_sub:
                booking_amount = float(booking.amount_paid or 0)
                commission_info = active_sub.deduct_booking(booking_amount)
        ...
        if commission_info:
            response_data['subscription'] = {
                'commission_charged': commission_info['commission'],
                'was_over_limit': commission_info['was_over_limit'],
            }
